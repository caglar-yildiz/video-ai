...
import { createOrganizationSchema, updateOrganizationSchema } from "@/validations/organizaion"
...
export async function PATCH (request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const body = await request.json()
    const { name, email } = updateOrganizationSchema.parse(body)

    const organization =user.organizationId ? await prisma.organization.update({
      where: {
        id:  user.organizationId,
      },
      data: {
    ...
    if (!organization) {
      return NextResponse.json({ error: "Error while updating organization" }, { status: 500 })
    }
...
export type PatchOrganizationResponse = {
  ...
  error?: "Unauthorized" | "Internal Server Error" | "Error while updating organization" | string;
};
